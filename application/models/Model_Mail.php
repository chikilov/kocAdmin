...
class Model_Mail extends CI_Model {
	...
	{
		parent::__construct();
		$this->DB = $this->load->database("koc_mail_master", TRUE);
		/**
	...
	*/
	...
	public function requestMaxPresent( $pid )
	{
		$query = "select count(idx) as cnt from koc_mail.mail where pid = ? and title = ? ";

		return $this->DB->query($query, array($pid, 'NG_MESSAGE_MAIL_REWARD_MAXLEVEL'));
	}

	public function getmaillist( $pid, $start_date, $end_date )
	{
		$query = "select title, attach_type, attach_value, expire_date, send_date, receive_date from koc_mail.mail where pid = ? and send_date between ? and ? order by send_date desc ";

		return $this->DB->query($query, array( $pid, $start_date.' 00:00:00.000', $end_date.' 23:59:59.999' ));
	}
}
?>
